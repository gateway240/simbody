...
public:
    ...
    virtual void solve( const Vector_<float>& b, Vector_<float>& x ) const {
       ...
       SimTK_APIARGCHECK_ALWAYS(false,"FactorLLT","solve",
   ...
   virtual void inverse(  Matrix_<double>& inverse ) const{
        SimTK_APIARGCHECK_ALWAYS(false,"FactorLLT","inverse",         "inverse(  <double> ) called with type that is inconsistent with the original matrix  \n");
    }
    virtual void inverse(  Matrix_<float>& inverse ) const{
        SimTK_APIARGCHECK_ALWAYS(false,"FactorLLT","inverse",         "inverse(  <float> ) called with type that is inconsistent with the original matrix  \n");
    }
    virtual void inverse(  Matrix_<std::complex<float> >& inverse ) const{
        SimTK_APIARGCHECK_ALWAYS(false,"FactorLLT","inverse",         "inverse(  std::complex<float> ) called with type that is inconsistent with the original matrix  \n");
    }
...
public:
    template <class ELT> FactorLLTRep( const Matrix_<ELT>&  );
    FactorLLTRep();
